...
-- Copiando estrutura para procedure space_ijp.proc_relatorioMissao
DROP PROCEDURE IF EXISTS `proc_relatorioMissao`;
DELIMITER //
CREATE PROCEDURE `proc_relatorioMissao`(
	IN `nRelatorio` INT
)
BEGIN
	if (numeroRelatorio = 1)
		then SELECT nomeCategoria FROM missoes WHERE nomeCategoria LIKE "%Refrigerante%"; 
		ELSE if (numeroRelatorio = 2)
				then SELECT COUNT(*) AS totalCategorias FROM missoes;
				ELSE if (numeroRelatorio = 3)
						then SELECT nomeCategoria FROM missoes ORDER BY nomeCategoria;
