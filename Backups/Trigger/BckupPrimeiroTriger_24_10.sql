...
-- Copiando estrutura para tabela space_ijp.auditoria
...
-- Copiando dados para a tabela space_ijp.auditoria: ~0 rows (aproximadamente)
...
INSERT INTO `financiamento` (`codFinanciamento`, `patrocinador`, `valor`, `Missoes_codMissao`) VALUES
	(1, 'nasa', 1000.00, 1);
...
CREATE TABLE IF NOT EXISTS `foguete` (
  ...
  `quantCombustivel` float NOT NULL,
  PRIMARY KEY (`codFoguete`)
) ENGINE=InnoDB AUTO_INCREMENT=2 DEFAULT CHARSET=utf8mb3;

-- Copiando dados para a tabela space_ijp.foguete: ~0 rows (aproximadamente)
INSERT INTO `foguete` (`codFoguete`, `nomeFoguete`, `maximoCombustivel`, `velocidade`, `status`, `quantCombustivel`) VALUES
	(1, 'Aguia', 5000, 80000.00, 0, 0);
...
-- Copiando estrutura para trigger space_ijp.tri_verificaStatusLancamento
DROP TRIGGER IF EXISTS `tri_verificaStatusLancamento`;
SET @OLDTMP_SQL_MODE=@@SQL_MODE, SQL_MODE='ONLY_FULL_GROUP_BY,STRICT_TRANS_TABLES,NO_ZERO_IN_DATE,NO_ZERO_DATE,ERROR_FOR_DIVISION_BY_ZERO,NO_ENGINE_SUBSTITUTION';
DELIMITER //
CREATE TRIGGER `tri_verificaStatusLancamento` BEFORE INSERT ON `lancamentos` FOR EACH ROW BEGIN
DECLARE statusFoguete INT;
DECLARE combustivelAtual FLOAT;
DECLARE combustivelMaximo FLOAT;

SELECT STATUS ,quantCombustivel, maximoCombustivel INTO statusFoguete, combustivelAtual, combustivelMaximo
FROM foguete
WHERE codFoguete = NEW.Foguete_codFoguete;

IF statusFoguete = 0 THEN
SIGNAL SQLSTATE '45000'
SET MESSAGE_TEXT = 'O foguete está em manutenção e não pode ser lançado.';
END IF;

IF statusFoguete = 2 THEN
SIGNAL SQLSTATE '45000'
SET MESSAGE_TEXT = 'O foguete já foi lançado e não pode ser lançado novamente.';
END IF;

IF statusFoguete = 3 THEN
SIGNAL SQLSTATE '45000'
SET MESSAGE_TEXT = 'O foguete está retornando para a base.';
END IF;

IF combustivelAtual < combustivelMaximo THEN
SIGNAL SQLSTATE '45000'
SET MESSAGE_TEXT = 'O foguete está sem combustível e não pode ser lançado.';
END IF;
END//
DELIMITER ;
